...
# ---------------------------------------------------------------------------
BASELINE_COST = 40_000_000.0
BUDGET = 44_000_000.0
N_SIMULATIONS = 100_000
SEED = 42

# ---------------------------------------------------------------------------
# 2. Pasta de saída — cria automaticamente se não existir
# ---------------------------------------------------------------------------
OUTPUT_DIR = "outputs"
...
# ---------------------------------------------------------------------------
# 3. Motor Monte Carlo + métricas
# ---------------------------------------------------------------------------
final_cost, contributions = simulate_cost_risk(
    df=risks, baseline_cost=BASELINE_COST, n_simulations=N_SIMULATIONS, seed=SEED,
)
metrics = summary_metrics(final_cost, baseline_cost=BASELINE_COST, budget=BUDGET)
drivers = risk_driver_table(contributions, final_cost)
risks_emv = add_emv(risks)

# ---------------------------------------------------------------------------
# 4. Gráficos (salvos como PNG dentro de outputs/)
# ---------------------------------------------------------------------------
fig1 = plot_histogram(final_cost, metrics, BASELINE_COST)
fig1.savefig(os.path.join(OUTPUT_DIR, "hist.png"), dpi=170)

fig2 = plot_s_curve(final_cost, budget=BUDGET)
fig2.savefig(os.path.join(OUTPUT_DIR, "scurve.png"), dpi=170)

fig3 = plot_risk_drivers(drivers)
fig3.savefig(os.path.join(OUTPUT_DIR, "drivers.png"), dpi=170)

# ---------------------------------------------------------------------------
# 5. Montagem do PDF
# ---------------------------------------------------------------------------
OUT_PATH = os.path.join(OUTPUT_DIR, "MVP-Risk.pdf")
...
premise_data = [
    ["Baseline cost", f"R$ {BASELINE_COST:,.0f}"],
...
extra_data = [
    ["Contingência P80 (vs. baseline)", f"R$ {metrics['p80_contingency']:,.0f}  "
     f"({metrics['p80_contingency']/BASELINE_COST:.1%} do baseline)"],
...
story.append(Image(os.path.join(OUTPUT_DIR, "hist.png"), width=16.5*cm, height=8.4*cm))
...
story.append(Image(os.path.join(OUTPUT_DIR, "scurve.png"), width=16.5*cm, height=8.4*cm))
story.append(Paragraph(
...
story.append(Image(os.path.join(OUTPUT_DIR, "drivers.png"), width=16.5*cm, height=8.4*cm))
story.append(Paragraph(
